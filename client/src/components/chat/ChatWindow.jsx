...
export default function ChatWindow() {
    ...
    const initiateCall = async(type) => {
        ...
        try {
            ...
            dispatch(setCallState({ active: true, type, stream, from: selectedChat._id, remoteName: selectedChat.fullName, remotePic: selectedChat.profilePic }));
            sendMessage(type === "video" ? "📹 Started a video call" : "📞 Started a voice call");
            socket.emit("call:initiate", { to: selectedChat._id, type, peerId: peerRef.current.id, callerName: user.fullName, callerPic: user.profilePic });
    ...
    const MoreOptionsMenu = ({ onClose, onClearChat, onBlockUser, onSearch, onMute, isMuted }) => (
        <>
            ...
            <div className="absolute top-16 right-4 sm:right-6 bg-white dark:bg-slate-800 shadow-2xl border border-slate-100 dark:border-slate-700 rounded-xl py-2 w-56 z-50 origin-top-right animate-in fade-in zoom-in-95 duration-200" onClick={(e) => e.stopPropagation()}>
                <button onClick={() => { onClearChat?.(); onClose(); }} className="w-full px-4 py-2.5 text-left hover:bg-slate-50 dark:hover:bg-slate-700 text-red-500 text-sm font-medium transition-colors">Clear Chat</button>
    ...
    return (
        <div className="h-full flex flex-col w-full bg-slate-50 dark:bg-slate-900 relative">
            {/* HEADER */}
            <div className="h-[70px] sm:h-[69px] bg-white dark:bg-slate-800 border-b dark:border-slate-700 px-4 sm:px-6 flex justify-between items-center z-10 relative shrink-0">
                {showSearchBar ? (
                    ...
                    <>
                        <div className="flex gap-3 items-center overflow-hidden">
                            ...
                            <div className="truncate">
                                ...
                                <p className={`text-xs ${isOnline ? "text-green-500" : "text-slate-400"}`}>{iBlocked ? "Blocked" : isOnline ? "Online" : "Offline"}</p>
                            </div>
                        </div>
                        <div className="flex gap-2 sm:gap-3 text-slate-600 dark:text-slate-200 items-center">
                            <button onClick={() => initiateCall("audio")} className="w-10 h-10 md:w-11 md:h-11 rounded-full bg-slate-100 dark:bg-slate-700 flex items-center justify-center hover:bg-slate-200 dark:hover:bg-slate-600 transition"><FiPhone size={18} /></button>
                            <button onClick={() => initiateCall("video")} className="w-10 h-10 md:w-11 md:h-11 rounded-full bg-slate-100 dark:bg-slate-700 flex items-center justify-center hover:bg-slate-200 dark:hover:bg-slate-600 transition"><FiVideo size={18} /></button>
                            ...
                            {showMenu && <MoreOptionsMenu onClose={() => setShowMenu(false)} onClearChat={handleClearChat} onBlockUser={handleBlockUser} onSearch={() => { setShowSearchBar(true); setShowMenu(false); }} onMute={handleMuteChat} isMuted={isChatMuted} />}
                        </div>
                    </>
                )}
            </div>
            ...
            {/* MESSAGES */}
            <div className="flex-1 overflow-y-auto relative bg-gradient-to-br from-slate-100 via-slate-50 to-slate-200 dark:from-slate-900 dark:via-slate-800 dark:to-slate-900 bg-[url('https://www.transparenttextures.com/patterns/cubes.png')] dark:bg-none bg-repeat relative">
                            ...
                            <div key={message._id} className="px-2">
                                {currentDate !== previousDate && (
                                    <div className="text-center my-4">
                                        <span className="bg-slate-200 dark:bg-slate-700 px-3 py-1 rounded-full text-xs text-slate-600 dark:text-slate-300">{currentDate}</span>
                                    </div>
                                )}
                                <MessageBubble
                                    messageObj={message}
            ...
            {/* INPUT AREA */}
            <div className="bg-white dark:bg-slate-800 border-t dark:border-slate-700 px-4 py-4 shrink-0">
                {iBlocked ? (
            ...
            {lightboxData.isOpen && (
                <div className="fixed inset-0 z-[9999] bg-black/95 flex items-center justify-center backdrop-blur-sm animate-in fade-in duration-200" onClick={closeLightbox}>
                    <button className="absolute top-6 right-6 text-white/70 hover:text-white p-2 transition bg-black/20 hover:bg-black/40 rounded-full" onClick={closeLightbox}>
                        <FiX size={28} />
                    </button>
                    {lightboxData.images.length > 1 && (
                        <>
                            <button className="absolute left-4 sm:left-10 text-white/70 hover:text-white p-3 transition bg-white/10 hover:bg-white/20 rounded-full backdrop-blur-md" onClick={handlePrevImage}>
                                <FiChevronLeft size={32} />
                            </button>
                            <button className="absolute right-4 sm:right-10 text-white/70 hover:text-white p-3 transition bg-white/10 hover:bg-white/20 rounded-full backdrop-blur-md" onClick={handleNextImage}>
                                <FiChevronRight size={32} />
                            </button>
                        </>
                    )}
                    <img src={lightboxData.images[lightboxData.currentIndex]} alt="lightbox view" className="max-w-[90vw] max-h-[85vh] object-contain select-none" onClick={(e) => e.stopPropagation()} />
                    {lightboxData.images.length > 1 && (
                        <div className="absolute bottom-8 bg-black/50 text-white/90 font-medium px-4 py-1.5 rounded-full text-sm tracking-wide">
